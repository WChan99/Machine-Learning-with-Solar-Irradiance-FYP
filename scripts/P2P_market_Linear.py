# Linear Regression combined with GUI and P2P Double Auction Market mechanism
...
while counter < 3:
    ...
    print(f"The date of the next day is {AI_test_Linear.next_day.strftime('%A , %d/%m/%Y')}.")
    ...
    for i in range(b):
            ...
            delivered_pwr_solar = AI_test_Linear.predictions[hour] * areas[i] * 0.15 * math.cos(math.radians(10)) / 10e6
  ...
  # Check if there is a trade between bid i and ask i
    ...
    if b == 0:
        # Solar Power graph
        y5 = np.array(AI_test_Linear.predictions)*0
        window = Solar_Graph.plot_graph0(x3,y5)
        print(window)
    elif b == 1:
        # Solar Power graph for 1
        y5 = np.array(AI_test_Linear.predictions)*areas[0]* 0.15 * math.cos(math.radians(10))/ 1e6
        window = Solar_Graph.plot_graph1(x3, y5, areas)    
    elif b == 2:
        # Solar Power graph for 2
        y5 = np.array(AI_test_Linear.predictions)*areas[0]* 0.15 * math.cos(math.radians(10))/ 1e6
        y6 = np.array(AI_test_Linear.predictions)*areas[1]* 0.15 * math.cos(math.radians(10))/ 1e6
        window = Solar_Graph.plot_graph2(x3, y5, y6, areas)
    elif b == 3:
        # Solar Power graph for 3
        y5 = np.array(AI_test_Linear.predictions)*areas[0]* 0.15 * math.cos(math.radians(10))/ 1e6
        y6 = np.array(AI_test_Linear.predictions)*areas[1]* 0.15 * math.cos(math.radians(10))/ 1e6
        y7 = np.array(AI_test_Linear.predictions)*areas[2]* 0.15 * math.cos(math.radians(10))/ 1e6
    ...
    def print_equilibrium(price, quantity_bid, quantity_ask, total_bids_quantity, total_asks_quantity, total_consumers, total_prosumers):
        output = f"{df}\n" \
                 f"{df2}\n" \
                 f"The date of the next day is {AI_test_Linear.next_day.strftime('%A , %d/%m/%Y')}.\n" \
                 f"The equilibrium price is ${price:.2f}\n" \
                 ...
                 f"The data from AI_test for Solar Irradiance for current hour is {AI_test_Linear.predictions[hour]:.2f}\n" \
        ...
        if total_consumers > total_prosumers:
            output += f"There is additional of {total_consumers-total_prosumers:.2f} units that will be supply by the battery.\n"
        elif total_consumers < total_prosumers:
            output += f"There is surplus of {total_prosumers-total_consumers:.2f} units that will be sold to the battery.\n"
        else:
            output += f"There is no surplus or additonal in overall.\n"

        layout = [[sg.Text(output, font=("Times New Roman", 18))]]
        window = sg.Window("P2P Market", layout)
    ...
    importlib.reload(AI_test_Linear)
    counter += 1 
